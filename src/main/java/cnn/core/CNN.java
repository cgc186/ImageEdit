...
import cnn.utils.MathUtils.Operator;
    ...
    public void train(DataSet trainset, int repeat) {
        ...
        for (int t = 0; t < repeat && !stopTrain.get(); t++) {
            ...
            if (trainset.size() % batchSize != 0)
                epochsNum++;
            System.out.println("第 " + t + " 次迭代，epochsNum: " + epochsNum);
            int right = 0;
            int count = 0;
            ...
            if (t % 10 == 1 && p > 0.96) {
                ...
                System.out.println("设置 alpha = " + ALPHA);
            }
            System.out.println("计算精度： " + right + count + " = " + p);
        }
