...
class FeaturesDump {
    ...
    private var featureDetector: FeatureDetector? = null
    private var descriptorExtractor: DescriptorExtractor? = null
    ...
    constructor(clusters: Int) {
        ...
        //bowDescriptorExtractor = BOWImgDescriptorExtractor(descriptorExtractor)
    ...
    // 聚类得出词典
    fun bulidVacab() {
        val vacabFs = opencv_core.FileStorage(DATA_FOLDER + "vocab.xml", opencv_core.FileStorage.READ)
        ...
        if (vacabFs.isOpened) {
            ...
            val fileStorage = opencv_core.FileStorage(DATA_FOLDER + "vocab.xml", opencv_core.FileStorage.WRITE)
    ...
    //构造BOW
    fun computeBowImage() {
        val tempVocabulary = opencv_core.FileStorage(DATA_FOLDER + "vocab.xml", opencv_core.FileStorage.READ)

        if (tempVocabulary.isOpened) {
            var tempVacab = Mat()
            //tempVacab = tempVocabulary["vocabulary"]
        ...
        if (file.exists()) {
            ...
            trainSet.forEach { matList ->
                ...
                matList.value.forEach {
                    ...
                    descriptorExtractor!!.compute(tempImage, kp, imageDescriptor)
    ...
    fun trainSvm() {
        ...
        for (k in 0 until categoriesSize) {
            val svmFil =
                opencv_core.FileStorage(DATA_FOLDER + categoryName[k] + "SVM.xml", opencv_core.FileStorage.READ)
            if (svmFil.isOpened) {
                continue
            } else {
                flag = -1
...
//                featureDetector!!.detectAndCompute(grayPic,noArray(),kp,test)
                ...
                for (i in 0 until categoriesSize) {
                    ...
                    val svmFile = opencv_core.FileStorage(fPath, opencv_core.FileStorage.READ)
                    if (svmFile.isOpened) {
